...
if ( empty( $_POST ) ) {
  ...
  foreach ( $produtoSelecionados as $k => $prod ) {
    ...
    $res = $sql->select( "CALL p_analise_produto_preco_Old (:CONTA,:FORNEC,:LOJA, :PRODUTOS)", array( ":CONTA" => $conta, ":FORNEC" => $fornec, ":LOJA" => $lojas, ":PRODUTOS" => $prod ) );
...
<div class="divBloco shadow">
  ...
  <input type="hidden" id="id_loja"  value="<?php echo $lojas;  ?>">
  <input type="hidden" id="id_forn"  value="<?php echo $fornec;  ?>">
  <div class="div_titulos size20"><?php echo $Class->getConta($conta) ." | ". $busca[0][0]['Loja'];?></div>
  Aliquota de Imposto:
